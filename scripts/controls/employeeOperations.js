...
const insertNewEmployee = (name, sector) => {
    return executeQuery("INSERT INTO employee (name, sector) VALUES (?,?)", [name, sector])
    .then(() => {return {sucess: true}})
    .catch(() => {return {sucess: false}})
}

const updateEmployee = (id, name, sector) => {
    return executeQuery("UPDATE employee SET name = ?, sector = ? WHERE id = ?", [name,sector,id])
    .then(() => {return {sucess: true}})
    .catch(() => {return {sucess: false}})
}

const deleteEmployee = (id) => {
    return executeQuery("DELETE FROM employee WHERE id = ?", [id])
    .then(() => {return {sucess: true}})
    .catch(() => {return {sucess: false}})
}
